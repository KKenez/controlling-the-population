...
async def create_generation(data: GenerateRequest, db: AsyncSession = Depends(get_db)):
    logger.info("POST /api/generation received: %s", data)
    try:
        week = await generate_week(data.routine_ids, data.week_start, db)
        logger.info("Generation succeeded: week_id=%s, events=%d", week.id, len(week.events))
    except ValueError as e:
        logger.error("Generation ValueError: %s", e)
